...
-- Mapping kiểu dữ liệu từ MySQL sakila gốc sang SQL Server DW:
--   INT UNSIGNED            → INT
--   TINYINT UNSIGNED        → SMALLINT  (rental_duration)
--   SMALLINT UNSIGNED       → INT       (film.length)
--   BOOLEAN                 → BIT       (active)
--   DECIMAL(5,2)/DECIMAL(4,2) → giữ nguyên
--   VARCHAR(n)              → NVARCHAR(n)
--   CHAR(20)                → NVARCHAR(20)
--   TEXT                    → NVARCHAR(MAX)
--   ENUM/SET                → NVARCHAR(10)
--   DATETIME/TIMESTAMP      → DATETIME
--   YEAR                    → INT
--   DATEPART() result       → INT
--   CASE WHEN 0/1 flag      → BIT
-- ============================================================
...
GO
INSERT INTO dw.Dim_Date VALUES(-1,'1900-01-01',0,N'Unknown',0,0,0,0,N'Unknown',0,0,0,0,N'Chua tra dia');
GO
PRINT '+ Dim_Date OK';
...
CREATE TABLE dw.Fact_Sale (
    ...
    rental_duration_expected    SMALLINT        NULL,
...
CREATE TABLE dw.Fact_Sale_byCustomer (
    agg_key INT NOT NULL IDENTITY(1,1), date_key INT NOT NULL, customer_key INT NOT NULL,
    store_key INT NOT NULL, staff_key INT NOT NULL, customer_class NVARCHAR(10) NULL,
    total_amount DECIMAL(10,2) NOT NULL DEFAULT 0, rental_count INT NOT NULL DEFAULT 0,
    late_count INT NOT NULL DEFAULT 0, CONSTRAINT PK_FSByCustomer PRIMARY KEY (agg_key)
); GO

IF OBJECT_ID('dw.Fact_Sale_byProduct','U') IS NOT NULL DROP TABLE dw.Fact_Sale_byProduct;
CREATE TABLE dw.Fact_Sale_byProduct (
    agg_key INT NOT NULL IDENTITY(1,1), date_key INT NOT NULL, product_key INT NOT NULL,
    store_key INT NOT NULL, total_amount DECIMAL(10,2) NOT NULL DEFAULT 0,
    rental_count INT NOT NULL DEFAULT 0, CONSTRAINT PK_FSByProduct PRIMARY KEY (agg_key)
); GO

IF OBJECT_ID('dw.Fact_Sale_byStore','U') IS NOT NULL DROP TABLE dw.Fact_Sale_byStore;
CREATE TABLE dw.Fact_Sale_byStore (
    agg_key INT NOT NULL IDENTITY(1,1), date_key INT NOT NULL, store_key INT NOT NULL,
    total_amount DECIMAL(10,2) NOT NULL DEFAULT 0, rental_count INT NOT NULL DEFAULT 0,
...
CREATE TABLE dw.Fact_Sale_byStaff (
    agg_key INT NOT NULL IDENTITY(1,1), date_key INT NOT NULL, store_key INT NOT NULL,
    staff_key INT NOT NULL, total_amount DECIMAL(10,2) NOT NULL DEFAULT 0,
    rental_count INT NOT NULL DEFAULT 0, CONSTRAINT PK_FSByStaff PRIMARY KEY (agg_key)
); GO
...
CREATE TABLE dw.Fact_Sale_byDate (
    agg_key INT NOT NULL IDENTITY(1,1), date_key INT NOT NULL, customer_key INT NOT NULL,
    store_key INT NOT NULL, total_amount DECIMAL(10,2) NOT NULL DEFAULT 0,
    rental_count INT NOT NULL DEFAULT 0, CONSTRAINT PK_FSByDate PRIMARY KEY (agg_key)
); GO
