-- ============================================================
-- FILE: 00_Create_All_Stored_Procedures.sql
-- CHẠY TRONG: SSMS, database Sakila_DW
-- MỤC ĐÍCH: Tạo tất cả SP để SSIS gọi bằng Execute SQL Task
-- ============================================================
...
BEGIN
    ...
    PRINT 'Load Dim_Geography_Store hoàn thành: '
        + CAST((SELECT COUNT(*) FROM dw.Dim_Geography_Store WHERE is_current=1) AS VARCHAR) + ' dòng';
...
BEGIN
    ...
    PRINT 'Load Dim_Staff hoàn thành: '
        + CAST((SELECT COUNT(*) FROM dw.Dim_Staff WHERE is_current=1) AS VARCHAR) + ' dòng';
...
BEGIN
    ...
    PRINT 'Load Dim_Customer hoàn thành: '
        + CAST((SELECT COUNT(*) FROM dw.Dim_Customer WHERE is_current=1) AS VARCHAR) + ' dòng';
...
BEGIN
    ...
    PRINT 'Load Dim_Product hoàn thành: '
        + CAST((SELECT COUNT(*) FROM dw.Dim_Product WHERE is_current=1) AS VARCHAR) + ' dòng';
...
BEGIN
    ...
    PRINT 'Load Fact_Sale hoàn thành: '
        + CAST((SELECT COUNT(*) FROM dw.Fact_Sale) AS VARCHAR) + ' dòng';
...
BEGIN
    SET NOCOUNT ON;
    PRINT 'Bắt đầu load Aggregate Facts...';
